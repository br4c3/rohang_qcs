...
class SIYISDK:
    ...
    #---------------------------새로 추가함--------------------------#
        try:
            maxWaitTime = float(maxWaitTime)   # ← 여기 추가
            maxRetries = int(maxRetries)       # ← 여기 추가
        except Exception as e:
            self._logger.error(f"[connect] Invalid argument type: {e}")
            return False
    #------------------------------------------------------------#
        """
        ...
        while retries < maxRetries:
            try:
                self._socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                self._socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # 추가
                self._socket.settimeout(self._rcv_wait_t)
    ...
    def disconnect(self):
        ...
        if self._socket:
            try:
                self._socket.sendto(b"exit", (self._server_ip, self._port))  # dummy 패킷 전송
                self._socket.close()
            except Exception as e:
                self._logger.error(f"Error closing socket: {e}")
            finally:
                self._socket = None # 소켓을 none으로 초기화해서 다음에 새로 만들도록
