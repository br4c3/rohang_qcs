...
class VisionTrackerNode(Node):
    ...
    def __init__(self):
        ...
        # ==== 1. ROS2 통신 설정 ====
        self.yaw_detection_pub = self.create_publisher(DetectionArray, '/yolo/detections', 10) 
        ...
        self.declare_parameter('qgc_video_width', 1920)
    ...
    def start_qgc_stream(self):
        gst_command = [
            ...
            'bframes=0','!','video/x-h264,profile=baseline','!',
        ...
        try:
            self.qgc_process = subprocess.Popen(gst_command,stdin=subprocess.PIPE)
            ...
            self.qgc_process = None
            self.get_logger().error(f"QGC 영상 송출기 실행 실패: {e}")
    ...
    def qgc_stream_worker(self):
        ...
        while not self.qgc_stop_event.is_set():
            ...
            if (process is None
                ...
                continue
            stream_frame = cv2.resize(frame,(self.qgc_width, self.qgc_height))
            try:
    ...
    def process_yolo_and_center_detection(self, frame, run_yolo):
        ...
        self.publish_image(frame, frame_header)  # yaw헤딩용 토픽
        ...
        if run_yolo and self.current_model is not None:
            ...
            self.publish_target(annotated_frame)

        self.enqueue_latest_qgc_frame(annotated_frame)

    def publish_target(self, annotated_frame):
        if self.target_info is None:
            return

        x1, y1, x2, y2, center_x, center_y = self.target_info
        
        ix1, iy1, ix2, iy2 = map(int, [x1, y1, x2, y2])
        icx, icy = int(center_x), int(center_y)
    ...
    def publish_image(self, frame, header):  # yaw헤딩용 토픽
        try:
    ...
    def publish_yaw_detections(self, boxes, header):  # yaw헤딩용 토픽
        ...
        self.target_info = (float(x1), float(y1), float(x2), float(y2), center_x, center_y)
        detection = Detection()
